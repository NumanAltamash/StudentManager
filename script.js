...
function createChart(labels,data){
  ...
  chart=new Chart(ctx,{
    type:"bar",
    data:{labels,datasets:[{label:"₹",data,backgroundColor:"#14b87a"}]}
  });
}
...
function createStudyChart(labels, data) {
  ...
  studyChart = new Chart(studyCtx, {
    type: "bar",
    data: { labels, datasets: [{ label:"Hours", data, backgroundColor:"#3b82f6"}] }
  });
}
...
function updateBudgetDashboard(){
  ...
  let percent = totalBudget>0 ? Math.round(((totalBudget-totalExpenses)/totalBudget)*100) : 0;
  percent = Math.max(0, Math.min(100, percent));

  document.getElementById("dash-budget-percent").innerText=percent+"%";
...
function updateBudgetUI(){
  ...
  categories.forEach(cat=>{
    ...
    grid.innerHTML+=`
      <div class="budget-box">
        ...
        <p class="budget-text">${percent.toFixed(0)}% used · ₹${Math.max(0,limit-spent).toFixed(2)} remaining</p>
      </div>`;
  });
  ...
  let totalPercent = totalBudget>0 ? Math.round((totalSpent/totalBudget)*100) : 0;
  ...
  studySessions.forEach((s,i)=>{
    table.innerHTML+=`
      <tr>
        ...
        <td>${s.hours}</td>
        <td>${s.note}</td>
        <td><button onclick="deleteStudy(${i})">Delete</button></td>
      </tr>
    `;
  });
}
...
function deleteStudy(index){
  if(index < 0 || index >= studySessions.length) return;
...
function editTodo(i){
  const newTask = prompt("Edit Task:", todos[i].task);
  if(newTask){
...
function addNote(){
  ...
  if(!title || !text){
    alert("Please enter title and note text!");
    return;
  }
  ...
  exams.forEach((ex, i) => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td style="font-weight:700;color:${daysLeft<=3 && daysLeft>0 ? '#ef4444':'#111'}">${ex.date ? daysText : "-"}</td>
        <td class="small-actions">
          <div style="display:flex;flex-direction:column;gap:8px;">
            <button class="small-btn" onclick="openEditExam(${i})">Edit</button>
            <button class="small-btn stop" onclick="promptDeleteExam(${i})">Delete</button>
          </div>
        </td>
      </tr>
    `;
...
function showWgPopup(message, type) {
  ...
  const txt = document.getElementById("wgPopupText");
